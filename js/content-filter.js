...
const waitForObjectTag = (resizableDiv) => {
  const objectTags = resizableDiv.getElementsByTagName("object");

  [...objectTags].forEach((object) => {
    object.addEventListener("load", function () {
      console.log("Object element loaded.");

      // Ensure the object's contentDocument is accessible
      const objectDoc = object.contentDocument;
      if (!objectDoc || !objectDoc.documentElement) {
        console.warn("Object content not accessible or empty.");
        console.warn("Object element:", object);
        return;
      }

      // Access the SVG element inside the object's contentDocument
      const svgElement = objectDoc.documentElement;

      // Add a click event listener to the SVG element
      svgElement.addEventListener("click", function () {
        // Request fullscreen for the SVG element
    ...
    object.addEventListener("error", function (event) {
      console.error("Error loading object element:", event);
    });
  });
};
